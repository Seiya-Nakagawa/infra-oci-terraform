...
def collect_news(db: Session = Depends(get_db)):
    ...
    try:
        ...
        articles = news_client.fetch_news(topics=["top"], max_articles=10)
        ...
        # 既存のダイジェストを確認
        existing_digest = (
            db.query(DailyDigest).filter(DailyDigest.date == today).first()
        )
        existing_headlines = existing_digest.headlines if existing_digest else []
        existing_links = {h.get("link") for h in existing_headlines}
        ...
        if not new_articles:
            return {
                ...
                "total_articles": len(existing_headlines),
                "added_count": 0,
            }
        ...
        # 既存のリストと結合（新着を上に）
        # セットの順序を維持しつつ最大30件に制限
        merged_headlines = (new_headlines + existing_headlines)[:30]

        # DailyDigestを保存または更新
        if existing_digest:
            existing_digest.headlines = merged_headlines
        ...
        return {
            ...
            "added_count": len(new_headlines),
            "total_articles": len(merged_headlines),
            "api_calls": 1,
        }
...
def get_daily_digest(
    target_date: Optional[str] = Query(
        None, description="対象日 (YYYY-MM-DD形式、省略時は今日)"
    ),
...
def list_news(db: Session = Depends(get_db)):
    ...
    today_digest = (
        db.query(DailyDigest).filter(DailyDigest.date == date.today()).first()
    )
